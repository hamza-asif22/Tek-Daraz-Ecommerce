...
const Add = ({ token }) => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setImage4(false)
        setPrice('')
        setBestseller(false)      // ✅ reset checkbox
        setSizes([])
      } else {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      <div className=''>
      <h2 className="text-2xl md:text-3xl font-bold mb-6 text-gray-800  pb-2">
        Add New Product
      </h2>
      </div>
      <div>
        <p className='mb-2 font-bold text-xl'>Product Images</p>
      ...
      <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
        ...
        <div>
          <p className='mb-2 font-bold text-xl'>Product price</p>
